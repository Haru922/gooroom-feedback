      ...
      char *argv[])
{
  ...
  setlocale (LC_ALL, "");
  bindtextdomain ("gooroom-feedback", "/usr/share/gooroom/locale");
  bind_textdomain_codeset ("gooroom-feedback", "UTF-8");
  textdomain ("gooroom-feedback");

  gooroom_feedback_app = gooroom_feedback_app_new ();
  status = g_application_run (G_APPLICATION (gooroom_feedback_app), argc, argv);
